...
class NeuralNetwork:
    ...
    def update_value_table(self, state, action, reward,done, new_state):
        if not done:
            value_update = self.value_table[state][action] + self.transition_probility * (reward + self.gamma * self.predict_value(new_state) - self.value_table[state][action])
        else:
            value_update = self.value_table[state][action] + self.transition_probility * (reward - self.value_table[state][action])
        self.value_table[state][action] = value_update
...
for episode in range(50000):
    ...
    while True:
        ...
        if done and reward != 1:
            reward -= .5
        # choices.append([state, action, reward, new_state])   
