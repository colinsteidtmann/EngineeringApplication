...
for layer_idx in range(len(layer_names)):
    if (layer_names[layer_idx] == "conv"):
        ...
        kernel_layers.append(np.ones((layer_filters_num, kernel_rows, kernel_cols, num_in_channels)))
        bias_layers.append(np.ones((layer_filters_num, 1, 1, 1)))
    else:
...
def conv_feedforward(input_layer):
    a = input_layer
    for idx, (layer_name, kernel_layer, bias_layer, stride_size, pool_size, pool_fn, activation_fn_str, padding_fn )in enumerate(zip(layer_names, kernel_layers, bias_layers, stride_sizes, pool_sizes, pool_fns, activation_fns, padding_fns)):
        if layer_name == "conv":
...
def conv_backprop(input_layer, delta_layer):
    ...
    return delta_layers[-1]


inputs = np.random.rand(2, 28, 28, 3)
first_output = conv_feedforward(inputs)
second_output = convnet.feedforward(inputs)
print(first_output[0, 0, 0, 0])
print(second_output[0,0,0,0])
delta = np.ones(conv_feedforward(inputs).shape) * 2
first_bias_derivs = np.array(conv_backprop(inputs, delta))
second_bias_derivs = np.array(convnet.gradients(inputs, [1], "input", delta)[0])
print(first_bias_derivs[0,0,0,0], second_bias_derivs[0,0,0,0])
