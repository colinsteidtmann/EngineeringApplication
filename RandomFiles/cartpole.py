...
import numpy as np
import random
import ApproxSine.neuralnetwork_ex7 as neuralnetwork
...
def get_q(model, observation):
  ...
  return model.feedforward(np_obs)
...
def train(model, observations, targets):
  ...
  model.SGD(zip(np_obs, np_targets), lr=LEARNING_RATE, lr_decay=0.01, epochs=1)
...
def get_model():
  model = neuralnetwork.NeuralNetwork(sizes=[4, 16, 16, 2], activation="relu", scale_method="", optimizer="nadam")
  ...
  for episode in range(NUM_EPISODES):
    ...
    #     env.render()
      ...
      if done:
        ...
        iteration_storage.append(iteration + 1)
        if (episode+1) >= 100:
            avg_iterations = (sum(iteration_storage[-100:])/100)
            print("avg iterations over past 100 episodes: ", avg_iterations)
            if avg_iterations >= 195:
                env.render()
