...
class NeuralNetwork:
    def __init__(self, env):
        ...
        self.onlineInputWeights = tf.Variable(tf.random_uniform((self.num_states, 16))
        self.onlineInputBias = tf.Variable(tf.random_uniform((1, 16))
        self.onlineOutputWeights = tf.Variable(tf.random_uniform((16, self.num_actions))
        self.onlineOutputBias = tf.Variable(tf.random_uniform((1, self.num_actions))
        
        self.targetInputWeights = tf.Variable(tf.random_uniform((self.num_states, 16)).assign(self.onlineInputWeights)
        self.targetInputBias = tf.Variable(tf.random_uniform((1, 16)).assign(self.onlineInputBias)
        self.targetOutputWeights = tf.Variable(tf.random_uniform((16, self.num_actions)).assign(self.onlineOutputWeights)
        self.targetOutputBias = tf.Variable(tf.random_uniform((1, self.num_actions)).assign(self.onlineOutputBias)

        self.target_update_time = 500
        self.epsilon = 1
    ...
    def train(self, batch):
        for state, action, reward, new_state, done in batch:
            ...
            weights1Gradient = np.dot(state.reshape((4,1)),np.dot(deriv_of_error,deriv_of_hiddenlayer))
            weights2Gradient = np.dot(deriv_of_hiddenlayer.reshape((16, 1)), deriv_of_error)
